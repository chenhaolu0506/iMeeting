...
class EquipRepair extends Component {
    ...
    insertOne = () => {
        ...
        let body = this.state.id === 0 ? JSON.stringify({
    ...
    userGetEquipRepairInfos = () => {
        const url = global.localhostUrl + "equip/userGetEquipRepairInfos";
        fetch(url, {
            method: "POST",
            mode: "cors",
            credentials: "include",
        ...
        const columns = [
            {
                ...
                render: (item) => {
                    let room = this.state.meetRoomList.find(room => room.id === item);
                ...
                dataIndex: "equipId",
                render: (item) => {
                    let equip = this.state.equipList.find(equip => equip.id === item);
