...
import { Button, Card, Row, Col, Drawer, Input, DatePicker, Modal, TimePicker, Form, Select, Table } from "antd";
import { SearchOutlined } from "@ant-design/icons";
import global from "../../../global";
import Highlighter from "react-highlight-words";
import Search from "antd/es/transfer/search";

const OneMeetingDrawer = Form.create({'name': 'form_in_modal'})(
    class extends Component {
        ...
        // 同意/批准
        agree = (ev, text) => {
            const url = global.localhostUrl + "meeting/agreeCoordinate?coordinateId=" + text;
            fetch(url, {
                method: "POST",
                mode: "cors",
                credentials: "include",
        ...
        // 拒绝
        disAgree = (ev, text) => {
            const url = global.localhostUrl + "meeting/disagreeCoordinate?coordinateId=" + text;
            fetch(url, {
                method: "POST",
                mode: "cors",
                credentials: "include",
        ...
        groupChange = (e) => {
            this.setState({
                ...
                this.state.selectedGroup.map((group, i) => {
                    const url = global.localhostUrl + "meeting/showOneGroup?groupId=" + group;
                    fetch(url, {
                        method: "POST",
                        mode: "cors",
                        credentials:"include",
        ...
        othersChange = (e) => {
            const L = this.state.othersList.filter((person, i) => {
                let flag = false
                e.map(name => person.name === person ? flag = true : null)
                console.log(i + ":" + flag)
                return flag;
            })
            this.setState({
        ...
        render() {
            ...
            const shouldDisplay = !this.props.coordinate || this.props.rob;
            return (
                <Drawer
                    title={this.props.changeAble?
                        (<Button href="#" type={"primary"} onClick={onCreate} disabled={!this.props.changeAble}>
                            {this.props.coordinate ? this.props.rob ? "抢占" : "申请调用" : "保存修改"}
                        </Button>)
                        :""
                    }
                    placement="right"
                        ...
                        <Form layout="vertical">
                                ...
                                <Col span={6}>
                                    <Form.Item label="日期">
                                        {getFieldDecorator('dateTime', {
                                            ...
                                            <DatePicker placeholder="选择日期" disabled={!this.props.changeAble}/>
                                        )}
                                    </Form.Item>
                                </Col>
                                <Col span={6}>
                                ...
                                <Col span={6}>
                                    <Form.Item label="准备时间">
                                        {getFieldDecorator('prepareTime', {
                                            ...
                                            <Select style={{ width: 140 }} onChange={()=>{}} disabled={!this.props.changeAble}>
                                                {timeList.map((time, i) => {
                                                    let T = "";
                                                    if(time < 60){
                                                        T = (time + "分钟")
                                                        ...
                                                        if(time % 60 === 0){
                                                            ...
                                                            T = (time / 60 - time / 60 % 1 + "小时 " + time % 60 + "分钟")
                                                        }
                                                    }
                                                    return(
                                                        <Select.Option value={time} key={i}>{T}</Select.Option>
                                                    )
                                                })}
                                            </Select>
                                        )}
                            ...
                            <Form.Item {...formItemLayout} label="说明">
                                {getFieldDecorator('description')(
                                    <Input.TextArea rows={5} disabled={!this.props.changeAble}/>
                                )}
                            </Form.Item>
                            <Form.Item {...formItemLayout} label="快速添加群组人员" style={{display:this.props.changeAble?"block":"none"}}>
                            ...
                            <Form.Item {...formItemLayout} label="参会人员">
                                {getFieldDecorator('guests',{
                                    ...
                                    >
                                        {
                                            this.state.departList.map((dept, i) => (
                                                <Select.OptGroup key={i} label={dept.name}>
                                                    {this.state.userList[i].map((user) => (
                            ...
                            <Row>
                                <Col span={18}>
                                    <Form.Item {...formItemLayout2} label="其它人员列表">
                                        {getFieldDecorator('others',{
                                            ...
                                            >
                                                {
                                                    this.state.othersList.map((user, i) => (
                                                        <Select.Option key={user.name} Option={user}>
                                                            {user.name}
                                                        </Select.Option>
                                                    ))
                                                }
                                ...
                                <Col span={4} offset={2}>
                                    <Button type={"primary"} onClick={this.showOthers} disabled={!this.props.changeAble}>
                                        快速添加
                                    </Button>
                                </Col>
                            </Row>
...
export default OneMeetingDrawer;