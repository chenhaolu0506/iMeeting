...
import { Table, Button, message, Tooltip, Input, Modal, Select } from "antd";
import { SearchOutlined, EyeOutlined, ExclamationOutlined, UploadOutlined, OrderedListOutlined, SnippetsOutlined, FileOutlined } from '@ant-design/icons';
import global from "../../../global";
import '../../../css/meeting.less';
import LeaveDrawer from '../../myMeeting/tools/LeaveDrawer';
...
import OutLineDrawerMe from "./OutLineDrawerMe";

class MyJoinMeetingInfo extends Component {
    componentDidMount() {
        this.reserveIndex();
    }
    state = {
    ...
    fileUpload = () => {
        ...
        const url = global.localhostUrl + "file/upload";
        fetch(url, {
            method: "POST",
            mode: "cors",
            credentials: "include",
    ...
    findByMeeting_file = (ev, id) => {
        const url = global.localhostUrl + "file/fineOneMeetingFileOnJoin?meetingId=" + id;
        fetch(url, {
            method: "POST",
            mode: "cors",
            credentials: "include",
    ...
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            const url = global.localhostUrl + "meeting/sendLeaveInformation";
            fetch(url, {
                method: "POST",
                mode: "cors",
                credentials: "include",
    ...
    render() {
                ...
                switch (text.status) {
                    case "未开始":
                        return (
                            <div>
                                <Tooltip title="查看">
                                    <Button onClick={(ev) => { this.showOneReserveDetail(ev, text.id, false) }}><EyeOutlined /></Button>
                                </Tooltip>
                                <Tooltip title="请假">
                                    <Button onClick={(ev) => { this.showOneLeaveDetail(ev, text.id, false) }}><ExclamationOutlined /></Button>
                                </Tooltip>
                                <Tooltip title="上传文件">
                                    <Button onClick={() => {
                                ...
                                <Tooltip title="查看会议大纲">
                                    <Button onClick={(ev) => { this.findByMeeting(ev, text.id) }}><OrderedListOutlined /></Button>
                                </Tooltip>
                                <Tooltip title="查看会议任务">
                                    <Button onClick={(ev) => { this.findByMeeting_task(ev, text.id) }}><SnippetsOutlined /></Button>
                                </Tooltip>
                                <Tooltip title="查看会议文件">
                                    <Button onClick={(ev) => { this.findByMeeting_file(ev, text.id, false) }}><FileOutlined /></Button>
                                </Tooltip>
                            </div>
                        )
                    default:
        ...
        return (
            <div >
                ...
                <Modal
                    title={"文件上传"}
                    open={this.state.fileUploadVisible}
                    onCancel={() => {
                        this.setState({
                            fileUploadVisible: false,
                        })
...
export default MyJoinMeetingInfo;